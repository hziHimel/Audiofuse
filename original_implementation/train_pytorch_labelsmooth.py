"""
AudioFuse — Label Smoothing variant (Direction 1.1)

Adds label smoothing (eps=0.1) to the BCE loss while keeping the pos_weight
to handle class imbalance. Smoothing softens hard 0/1 targets to improve
calibration and generalization.

Usage:
    python train_pytorch_labelsmooth.py --train_csv data/train.csv --val_csv data/val.csv \
                                         --seed 1 --output_dir outputs/pytorch_labelsmooth/
"""
...
from losses import bce_with_label_smoothing
...
def run_epoch(model, loader, optimizer, pos_weight, train=True):
    ...
    with torch.set_grad_enabled(train):
        for spec, wave, labels in tqdm(loader, leave=False):
            ...
            # Label smoothing is applied first, then pos_weight is incorporated by
            # scaling the smoothed-target BCE — achieved via manual weighted reduction.
            smooth_targets = labels * (1.0 - LABEL_SMOOTH_EPS) + (1.0 - labels) * LABEL_SMOOTH_EPS
...
def train_one_seed(args, seed: int):
    ...
    print(f"pos_weight = {pos_weight.item():.4f}, label_smooth_eps = {LABEL_SMOOTH_EPS}")
    ...
    print(f"\n{'='*50}")
    print(f"Seed {seed} Results [LABEL SMOOTH eps={LABEL_SMOOTH_EPS}] (threshold=0.50):")
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/pytorch_labelsmooth/")
    ...
    for seed in args.seeds:
        print(f"\n{'#'*60}\n# Training with seed={seed} [LABEL SMOOTHING eps={LABEL_SMOOTH_EPS}]\n{'#'*60}")
