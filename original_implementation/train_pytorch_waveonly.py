"""
AudioFuse — Waveform-Only Baseline (Direction 3.1 / pretrained branch init)

Trains only the waveform CNN branch with a classification head.
Two purposes:
  1. Clean single-branch ablation number for the paper.
  2. Saves pretrained CNN weights for use in pretrained-branch-init experiment.

Saved checkpoint (best_seed{N}.pt) contains only the WaveformCNN + head weights
and can be loaded into AudioFuse.wave_branch for pretrained init.

Usage:
    python train_pytorch_waveonly.py --train_csv data/train.csv \
        --val_csv data/val.csv --seeds 1 --output_dir outputs/pytorch_waveonly/
"""
...
from train_pytorch import Config, WaveformCNN, sweep_threshold, DEVICE

C = Config()


class WaveDataset(Dataset):
    """Loads raw waveform only — no spectrogram needed."""
...
class WaveClassifier(nn.Module):
    """WaveformCNN + classification head for single-branch training."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, wave):
        return self.head(self.wave_branch(wave)).squeeze(1)
...
def train_one_seed(args, seed: int):
    ...
    train_loader = DataLoader(WaveDataset(train_df), batch_size=C.BATCH_SIZE, shuffle=True,
                              num_workers=2, pin_memory=True)
    val_loader = DataLoader(WaveDataset(val_df), batch_size=C.BATCH_SIZE, shuffle=False,
                            num_workers=2, pin_memory=True)

    n_neg = (train_df["label"] == 0).sum()
    n_pos = (train_df["label"] == 1).sum()
    pos_weight = torch.tensor([n_neg / n_pos], dtype=torch.float32).to(DEVICE)
    print(f"pos_weight = {pos_weight.item():.4f}  [waveform-only CNN]")
    ...
    print(f"\n{'='*50}")
    print(f"Seed {seed} Results [WAVE-ONLY] (threshold=0.50):")
    ...
    print(f"{'='*50}")
    print(f"\nPretrained CNN weights saved to: {best_ckpt}")
    print("Load into AudioFuse with: model.wave_branch.load_state_dict(torch.load(ckpt)['wave_branch'])")
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/pytorch_waveonly/")
    ...
    for seed in args.seeds:
        print(f"\n{'#'*60}\n# Training with seed={seed} [WAVE-ONLY]\n{'#'*60}")
