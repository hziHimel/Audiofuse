"""
AudioFuse — MFCC Spectrogram Input (Direction 1.2)

Replaces log-Mel spectrogram with MFCC (n_mfcc=40) as input to the ViT branch.
MFCC is computed on-the-fly from the wav file (no separate preprocessing step).
The (40, T) MFCC matrix is bilinearly resized to (224, 224) to match ViT input.
Waveform branch and fusion head are identical to baseline.

Motivation: given that the ViT branch contributes almost nothing with log-Mel
(branch ablation AUC=0.4588), MFCC may provide a more discriminative spectral
representation that the ViT can learn from.

Usage:
    python train_pytorch_mfcc.py --train_csv data/train.csv \
        --val_csv data/val.csv --seeds 1 --output_dir outputs/pytorch_mfcc/
"""
...
from train_pytorch import Config, AudioFuse, sweep_threshold, DEVICE
...
class MFCCDataset(Dataset):
    """Loads MFCC (40 coeffs, resized to 224×224) + raw waveform."""
    ...
    def __getitem__(self, idx):
        ...
        # MFCC: (N_MFCC, T) → normalize → resize to (1, 224, 224)
        mfcc = librosa.feature.mfcc(y=wav, sr=C.SR, n_mfcc=N_MFCC,
                                     n_fft=2048, hop_length=512)
        mn, mx = mfcc.min(), mfcc.max()
        mfcc = (mfcc - mn) / (mx - mn + 1e-6)
        mfcc_t = torch.from_numpy(mfcc.astype(np.float32)).unsqueeze(0).unsqueeze(0)  # (1,1,40,T)
        spec = F.interpolate(mfcc_t, size=(C.IMG_SIZE, C.IMG_SIZE),
...
def train_one_seed(args, seed: int):
    ...
    train_loader = DataLoader(MFCCDataset(train_df), batch_size=C.BATCH_SIZE, shuffle=True,
                              num_workers=2, pin_memory=True)
    val_loader = DataLoader(MFCCDataset(val_df), batch_size=C.BATCH_SIZE, shuffle=False,
                            num_workers=2, pin_memory=True)

    n_neg = (train_df["label"] == 0).sum()
    n_pos = (train_df["label"] == 1).sum()
    pos_weight = torch.tensor([n_neg / n_pos], dtype=torch.float32).to(DEVICE)
    print(f"pos_weight = {pos_weight.item():.4f}  [MFCC n_mfcc={N_MFCC}, resized to 224×224]")
    ...
    print(f"\n{'='*50}")
    print(f"Seed {seed} Results [MFCC] (threshold=0.50):")
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/pytorch_mfcc/")
