"""
AudioFuse — On-the-fly Audio Augmentation (Direction 1.4)

Identical to baseline except training batches receive:
  - Gaussian noise on waveform (SNR=20 dB)
  - SpecAugment on spectrogram (2 time masks T≤40, 2 freq masks F≤20)

Augmentations are applied after the batch is moved to device (no CPU overhead).
Val set is never augmented.

Usage:
    python train_pytorch_augment.py --train_csv data/train.csv \
        --val_csv data/val.csv --seeds 1 --output_dir outputs/pytorch_augment/
"""
...
def run_epoch(model, loader, optimizer, pos_weight,
              noise_aug, spec_aug, train=True):
    model.train(train)
    noise_aug.train(train)
    spec_aug.train(train)
...
def train_one_seed(args, seed: int):
    ...
    print(f"pos_weight = {pos_weight.item():.4f}  [augment: GaussianNoise(SNR=20dB) + SpecAugment]")

    model = AudioFuse().to(DEVICE)
    noise_aug = GaussianNoise(snr_db=20).to(DEVICE)
    spec_aug  = SpecAugment(T=40, F=20, num_time_masks=2, num_freq_masks=2).to(DEVICE)
    ...
    for epoch in range(1, C.EPOCHS + 1):
        tr_loss, tr_acc, tr_auc = run_epoch(
            model, train_loader, optimizer, pos_weight, noise_aug, spec_aug, train=True)
        vl_loss, vl_acc, vl_auc = run_epoch(
            model, val_loader, optimizer, pos_weight, noise_aug, spec_aug, train=False)
    ...
    print(f"\n{'='*50}")
    print(f"Seed {seed} Results [AUGMENT] (threshold=0.50):")
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/pytorch_augment/")
    ...
    for seed in args.seeds:
        print(f"\n{'#'*60}\n# Training with seed={seed} [AUGMENT]\n{'#'*60}")
