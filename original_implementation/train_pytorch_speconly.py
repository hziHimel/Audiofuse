"""
AudioFuse — Spectrogram-Only (ViT) Baseline (Direction 3.1 / pretrained branch init)

Trains only the ViT branch with a classification head on log-Mel spectrograms.
Two purposes:
...
Usage:
    python train_pytorch_speconly.py --train_csv data/train.csv \
        --val_csv data/val.csv --seeds 1 --output_dir outputs/pytorch_speconly/
"""
...
class SpecClassifier(nn.Module):
    """SpectrogramViT + classification head for single-branch training."""
    def __init__(self):
        super().__init__()
...
def train_one_seed(args, seed: int):
    ...
    print(f"{'='*50}")
    print(f"\nPretrained ViT weights saved to: {best_ckpt}")
    print("Load into AudioFuse with: model.spec_branch.load_state_dict(...)")
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/pytorch_speconly/")
